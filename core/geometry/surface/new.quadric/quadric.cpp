	...
	:Surface("QUADRIC", name)
{
     ...
     *
     * 作業：
     * コンストラクタで標準化空間への変換行列を求めて、matrix_, invMatrix_に追加しておく。matrixToLocal
     * これによりクロスタームの係数 D_ E_ F_ は０になる。
     *
     * その前に、matrix_はインスタンス作成時に適用して係数に反映させ、使用しないようにしたほうが
     * わかりやすくなるのではないか？
     */
...
void geom::Quadric::transform(const math::Matrix<4> &matrix)
{
    ...
    double nF = 2*a11*a13*A_ + 2*a21*23*B_ + 2*a31*a33*C_
							 ...
							 bool warnPhitsCompat)
{
	...
	case Quadric::TYPE::GQ:
		...
		break;
//	default:
//		mFatal(std::string("quadric.cpp:"), std::to_string(__LINE__), " Invalid type");
	}
