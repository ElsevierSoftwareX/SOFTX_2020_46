...
public:
	...
	//static CGZone fromQadFixedString(const std::string &str);
	static std::vector<CGZone> getCgZonesFromQadFixed(std::stringstream &ss, int &lineNumber);
	static std::vector<CGZone> getCgZonesFromMarsFixed(std::stringstream &ss, int &lineNumber);
	// strが継続データならtrueを返す。
	static bool isContData(const std::string &str);
	static bool isEndString(const std::string &str);
